<script lang="ts">
  ...
  const OVERLAYS: Overlay[] = [
    ...
    {
      // Now Playing transport (bcfeb03): L/R across the actions, A/Enter activates, B/Esc
      // closes. Sits right after the deck, matching the pre-roster handler's precedence.
      open: () => npOpen,
      key: (e) => {
      ...
      pad: (c) => {
        if (c === "DPadLeft") holdStart(c, () => npMove(-1));
        else if (c === "DPadRight") holdStart(c, () => npMove(1));
        else if (c === "South") npActivate();
        else if (c === "East" || c === "Start") npOpen = false;
      },
      stickX: (d) => npMove(d), // horizontal action row — vertical stick is meaningless here
    },
    {
      // First-run wizard
      open: () => wizardActive,
